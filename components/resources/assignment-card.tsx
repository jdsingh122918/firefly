...
interface AssignmentCardProps {
  ...
  onStatusChange?: (assignment: ResourceAssignment, newStatus: AssignmentStatus) => void;
  showResource?: boolean; // Whether to show resource information
  showActions?: boolean;
  className?: string;
}

const getAssignmentCardColors = (assignment: ResourceAssignment, isOverdue: boolean) => {
...
export function AssignmentCard({
  ...
  const handleQuickStatusChange = (newStatus: AssignmentStatus) => {
    if (onStatusChange) {
  ...
  const handleViewResource = () => {
    if (assignment.resource) {
      ...
      router.push(`/${rolePrefix}/resources/${assignment.resource.id}`);
    }
  };
  ...
  return (
    <Card
      className={cn(
        ...
        cardColors.hover,
        assignment.status === AssignmentStatus.COMPLETED && "opacity-75",
        className
      )}
    >
          ...
          {showActions && (canEdit || canChangeStatus) && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {canChangeStatus && assignment.status !== AssignmentStatus.COMPLETED && (
                  <>
                    {assignment.status === AssignmentStatus.ASSIGNED && (
                      <DropdownMenuItem
                        onClick={() => handleQuickStatusChange(AssignmentStatus.IN_PROGRESS)}
                      >
                    ...
                    {assignment.status === AssignmentStatus.IN_PROGRESS && (
                      <DropdownMenuItem
                        onClick={() => handleQuickStatusChange(AssignmentStatus.COMPLETED)}
                      >
      ...
      <CardContent className="pt-0 space-y-3">
        ...
        {showResource && assignment.resource && (
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {assignment.resource.resourceType} Resource
                </p>
              </div>
            </div>
          </div>
        ...
        {/* Assignment notes - TODO: Add notes field to ResourceAssignment model if needed */}
        {/* {assignment.notes && (
        ...
        {assignment.status !== AssignmentStatus.COMPLETED && (
          <div className="pt-2 border-t">
            <div className="flex gap-2 flex-wrap">
              {/* Complete Assignment - Direct navigation to completion page (for assignees) */}
              {canStartWork && (
                <Button
                  size="sm"
                  onClick={() => router.push(`/member/assignments/${assignment.id}/complete`)}
                  className="min-h-[44px] flex-1 sm:flex-none font-medium"
                >
              ...
              {canStartWork && assignment.status === AssignmentStatus.ASSIGNED && (
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => handleQuickStatusChange(AssignmentStatus.IN_PROGRESS)}
              ...
              {/* Quick mark complete - for when user doesn't need to fill form */}
              {canChangeStatus && (assignment.status === AssignmentStatus.IN_PROGRESS ||
                assignment.status === AssignmentStatus.ASSIGNED) && !canStartWork && (
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => handleQuickStatusChange(AssignmentStatus.COMPLETED)}
        ...
        {assignment.status === AssignmentStatus.COMPLETED && assignment.completedAt && (
          <div className="pt-2 border-t">
            ...
            {/* View completed assignment for assignees */}
            {canStartWork && (
              <Button
                size="sm"
                variant="outline"
