...
interface AssignmentFormProps {
  ...
  onSuccess?: (assignment: ResourceAssignment) => void;
  className?: string;
}

interface FormData {
  selectedResourceId?: string; // For create mode when resource needs to be selected
  assignedTo: string;
  priority: AssignmentPriority;
  dueDate: Date | null;
  // notes: string; // TODO: Add notes field to ResourceAssignment model if needed
  status?: AssignmentStatus; // Only for edit mode
}
...
export function AssignmentForm({
  ...
  const [formData, setFormData] = useState<FormData>(() => ({
    ...
    // notes: assignment?.notes || "", // TODO: Add notes field to ResourceAssignment model if needed
  ...
  const validateForm = useCallback((): string | null => {
    // Check if we have a valid resource ID in create mode
    if (mode === "create" && !resourceId && !formData.selectedResourceId) {
      return "Please select a resource to create an assignment for";
    }
    if (!formData.assignedTo) {
      return "Please select a user to assign this task to";
    }
    // TODO: Add notes validation when notes field is added to ResourceAssignment model
    // if (formData.notes.length > 500) {
    //   return "Notes must be less than 500 characters";
    // }
    return null;
  }, [formData, mode, resourceId]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (mode === "create") {
        const targetResourceId = resourceId || formData.selectedResourceId;
        if (!targetResourceId) {
          throw new Error("Resource ID is required for creating assignments");
        }
        url = `/api/assignments`;
        method = "POST";
        payload = {
          ...
          // notes: formData.notes.trim() || undefined, // TODO: Add notes field to ResourceAssignment model
        };
      } else {
        if (!assignment?.id) {
      ...
      if (mode === "create") {
        setFormData({
          ...
          dueDate: null,
          // notes: "", // TODO: Add notes field to ResourceAssignment model
        });
      }
    } catch (err) {
  ...
  }, [formData, validateForm, getToken, mode, resourceId, assignment, onSuccess, onOpenChange]);
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className={`max-w-2xl max-h-[90vh] overflow-y-auto ${className}`}>
        <DialogHeader>
          ...
          {resourceTitle && (
            <p className="text-sm text-muted-foreground mt-1">
              For resource: <span className="font-medium">{resourceTitle}</span>
            </p>
          )}
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {/* Select Resource (when resourceId not provided) */}
          {mode === "create" && !resourceId && (
            <div className="space-y-2">
              <Label className="text-sm font-medium">
                Resources *
              </Label>
              <ResourceCombobox
                value={formData.selectedResourceId}
                onValueChange={(value) => handleInputChange("selectedResourceId", value || "")}
                placeholder="Search and select a resource..."
                sourceFilter={["system", "user", "public"]}
                className="w-full"
              />
              <p className="text-xs text-muted-foreground">
                Search for the resource you want to create an assignment for.
              </p>
            </div>
          )}
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                <Select
                  ...
                  onValueChange={(value) => handleInputChange("status", value as AssignmentStatus)}
                >
                  <SelectTrigger className="w-40 min-h-[44px]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value={AssignmentStatus.ASSIGNED}>Assigned</SelectItem>
          ...
          {/* Notes - TODO: Add notes field to ResourceAssignment model if needed */}
          {/* <div className="space-y-2">
