import { auth } from '@clerk/nextjs/server';
import { redirect } from 'next/navigation';
import { PrismaClient, UserRole } from '@prisma/client';
import ContentEditPage from '@/components/content/content-edit-page';

/**
 * Member Content Edit Page
 *
 * Allows MEMBER users to edit content they have access to:
 * - Edit own created content only
 * - Limited metadata modification
 * - No family assignment (members don't manage families)
 * - Basic content features only
 * - No curation controls
 */

const prisma = new PrismaClient();

interface MemberContentEditPageProps {
  params: Promise<{ id: string }>;
}

export default async function MemberContentEditPageComponent({ params }: MemberContentEditPageProps) {
  const { id } = await params;
  const { userId, sessionClaims } = await auth();

  if (!userId) {
    redirect('/sign-in');
  }

  // Get user role and database ID with dual-path pattern
  const userRole = (sessionClaims?.metadata as { role?: UserRole })?.role;
  let finalUserRole = userRole;
  let dbUserId: string | undefined;

  // Get user's database record for ID and role
  const dbUser = await prisma.user.findUnique({
    where: { clerkId: userId },
    select: { id: true, role: true }
  });

  if (dbUser) {
    dbUserId = dbUser.id;
    if (!finalUserRole) finalUserRole = dbUser.role as UserRole;
  }

  if (finalUserRole !== UserRole.MEMBER) {
    redirect('/unauthorized');
  }

  if (!dbUserId) {
    redirect('/admin/debug'); // User needs to sync their account
  }

  // Fetch available categories (members don't have access to families for content)
  const categories = await prisma.category.findMany({
    where: { isActive: true },
    select: { id: true, name: true, color: true },
    orderBy: { name: 'asc' }
  });

  // Transform categories to handle null vs undefined for color
  const transformedCategories = categories.map(category => ({
    ...category,
    color: category.color ?? undefined
  }));

  // Members don't manage families
  const families: Array<{ id: string; name: string }> = [];

  return (
    <ContentEditPage
      contentId={id}
      userRole={finalUserRole}
      userId={userId}
      availableFamilies={families}
      availableCategories={transformedCategories}
      showFamilySelector={false} // Members can't assign content to families
      showAssignmentManagement={false} // Members can't manage assignments
      showCurationControls={false}
      allowContentTypeChange={false} // Members can't change content type after creation
    />
  );
}